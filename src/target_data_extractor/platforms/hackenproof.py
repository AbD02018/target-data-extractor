"""HackenProof platform extractor.

HackenProof is a public bug bounty platform. It has a public programs listing at
hackenproof.com/programs and per-program pages. No public API documented.
"""
...
class HackenProofPlatform(BasePlatform):
    platform_name = "hackenproof"
    hostnames = ["hackenproof.com"]
    requires_auth = False
    ...
    async def extract(self, url: str, **kwargs: Any) -> BountyProgram:
        ...
        candidates = [
            ...
            f"{self.BASE_URL}/bounties/{handle}",
            url,
        ]
        html = ""
        for u in candidates:
            try:
                ...
                logger.debug("hackenproof fetch %s failed: %s", u, exc)
        if not html:
            raise ParseError(f"Could not fetch HackenProof page for {handle}", platform=self.platform_name)
    ...
    def _extract_state(self, soup: Any) -> dict[str, Any]:
        for script in soup.find_all("script"):
            ...
            for marker in ("__NEXT_DATA__", "window.__NUXT__", "window.__PRELOADED_STATE__", "window.__INITIAL_STATE__"):
                if marker in text:
    ...
    def _build_program(self, data: dict[str, Any], handle: str, source_url: str) -> BountyProgram:
        ...
        program = (
            ...
            or deep_get(data, "props", "pageProps", "bounty")
        ...
        description = program.get("description") or program.get("brief") or program.get("overview")
        ...
        for t in targets if isinstance(targets, list) else []:
            ...
            is_in = bool(t.get("in_scope", True) or t.get("eligible", True))
        ...
        rewards = program.get("rewards") or program.get("bounty_table") or {}
        if isinstance(rewards, dict):
            bounty_table, min_amt, max_amt = self._bounty_table_from_dict(rewards)
        try:
            if not max_amt:
                raw_max = program.get("max_reward") or program.get("maxBounty") or program.get("reward_max")
                if raw_max:
        ...
        rules = ProgramRules(
            ...
            submission_format="Report via HackenProof platform",
